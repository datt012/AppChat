...
wss.on('connection', (ws) => {
    ...
    wsSend('notifi-connected', client_uuid, nickname, connect_message);
            ...
            if(msg.indexOf('/nick') === 0) {
                ...
                if(nickname_array.length >= 2) {
                    ...
                    let nickname_message = 
                        'Client ' + old_nickname + ' changed name to ' + nickname;
    ...
    var closeSocket = (customMessage) => {
        for (var i = 0; i < clients.length; i++) {
            if(clients[i].id == client_uuid) {
                ...
                wsSend('notifi-disconnected', client_uuid, nickname, disconnect_message);
                clients.splice(i, 1);
            }
        }
    };
